...
import { useSectionItemDeleteStore } from '../../stores/useSectionItemDeleteStore'
import { getSectionItemsColumns } from './section-items-columns'
...
interface SectionItemsTableProps {
  /** Cuando viene, la tabla queda fija a esta sección (uso embebido, ej. en el detalle de Section) — el filtro de sección no se muestra en la UI, se aplica siempre en el load(). */
  idSection?: number
  /** `type_section.web_settings.items_delete` — oculta "Eliminar" (por fila y masivo). Default
   * `true` (comportamiento de siempre). Crear se controla aparte, ocultando el botón "Nuevo
   * item" en `SectionDetailItemsTab`. */
  canDelete?: boolean
  /** `type_section.web_settings.items_reorder` — oculta la columna "Orden". Default `true`
   * (comportamiento de siempre). */
  canReorder?: boolean
}

export function SectionItemsTable({ idSection, canDelete = true, canReorder = true }: SectionItemsTableProps) {
  const {
    items, meta, filters, hasLoaded, isInitialLoading, isFetching, isError, message,
    load, reset,
  } = useSectionItemListStore()
  const { bulkToggleState, bulkDeleteItems } = useSectionItemDeleteStore()

  const columns = useMemo(() => getSectionItemsColumns({ hideDelete: !canDelete, hideOrder: !canReorder }), [canDelete, canReorder])
  ...
  useEffect(() => { void load({ id_section: idSection, page: 1 }) }, [idSection])
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ...
      void load({ id_section: idSection, search, type: type === 'all' ? undefined : type, state: state === 'all' ? undefined : Number(state), page: 1 }).finally(() => setIsUserFetching(false))
    }, 500)
    return () => window.clearTimeout(t)
  }, [search])

  // Tipo/Estado son acciones discretas (una selección), no texto que se esté escribiendo: se
  // disparan de inmediato, sin esperar el debounce de "Buscar".
  const applyFilters = (overrides: { type?: string; state?: string }) => {
    const nextType = overrides.type ?? type
    const nextState = overrides.state ?? state
    setIsUserFetching(true)
    void load({ id_section: idSection, search, type: nextType === 'all' ? undefined : nextType, state: nextState === 'all' ? undefined : Number(nextState), page: 1 }).finally(() => setIsUserFetching(false))
  }
  ...
  const table = useReactTable({
    ...
    onPaginationChange: (updater) => {
      ...
      setIsUserFetching(true)
      void load({ id_section: idSection, page: next.pageIndex + 1, per_page: next.pageSize }).finally(() => setIsUserFetching(false))
    },
  ...
  const resetFilters = () => {
    ...
    setSearch(''); setType('all'); setState('all')
    setIsUserFetching(true)
    void load({ id_section: idSection, search: '', type: undefined, state: undefined, page: 1 }).finally(() => setIsUserFetching(false))
  }
  ...
  if (isError) {
    return (
      <ErrorState
        ...
        onPrimaryAction={() => {
          reset();
          void load({ id_section: idSection })
        }}
      />
    )
  }
      ...
      <div className="flex items-end justify-between gap-2">
        <div className="flex flex-1 flex-wrap items-end gap-2">
          <div className="flex flex-col gap-1">
            ...
            <Input placeholder="Título, label, clave..." value={search} disabled={isFetching} onChange={(e) => setSearch(e.target.value)} className="h-8 w-full sm:w-[220px]" />
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-xs text-muted-foreground">Tipo</span>
            <Select value={type} disabled={isFetching} onValueChange={handleTypeChange}>
              <SelectTrigger className="h-8 w-full sm:w-[160px]"><SelectValue placeholder="Tipo" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Todos los tipos</SelectItem>
                {SECTION_ITEM_TYPES.map((t) => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="flex flex-col gap-1">
