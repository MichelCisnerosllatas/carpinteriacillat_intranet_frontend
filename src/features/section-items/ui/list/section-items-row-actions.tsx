...
interface SectionItemsRowActionsProps {
  row: Row<SectionItem>
  /** El tipo de sección de este item tiene `web_settings.items_delete: false` — no se puede eliminar, solo editar texto/visibilidad. Ver `SectionDetailItemsTab`. */
  hideDelete?: boolean
}

export function SectionItemsRowActions({ row, hideDelete = false }: SectionItemsRowActionsProps) {
  const router = useRouter()
  const { setCurrentItem } = useSectionItemListStore()
  const { toggleState, deleteItem } = useSectionItemDeleteStore()

  const isActive = row.original.stateValue === 1
  const label    = row.original.title || row.original.label || `Item #${row.original.id}`

  const handleView = () => {
    setCurrentItem(row.original)
    NProgress.start()
    router.push(`/section-items/${row.original.id}`)
  }

  const handleEdit = () => {
    setCurrentItem(row.original)
    NProgress.start()
    router.push(`/section-items/edit/${row.original.id}`)
  }
