// src/features/typesections/ui/detail/typesection-detail.tsx
'use client'

import { useEffect } from 'react'
import { useRouter } from 'next/navigation'
import { Pencil, LayoutGrid, CalendarDays } from 'lucide-react'
...
export function TypeSectionDetail({ id }: { id: string }) {
  const router = useRouter()
  const { currentItem, items, setCurrentItem } = useTypeSectionListStore()

  useEffect(() => {
    if (!currentItem || String(currentItem.id) !== id) {
      const found = items.find((i) => String(i.id) === id)
      if (found) setCurrentItem(found)
      else router.replace('/typesections')
    }
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-start gap-4">
            ...
            <Button variant="outline" size="sm" onClick={() => { NProgress.start(); router.push(`/typesections/edit/${item.id}`) }}>
