// src/features/typesections/ui/form/typesection-form.tsx
'use client'
...
export function TypeSectionForm({ mode, id }: { mode: 'create' | 'edit'; id?: string }) {
  const router = useRouter()
  const { currentItem, items }                                          = useTypeSectionListStore()
  const { isSubmitting, error, fieldErrors, create, update, reset }     = useTypeSectionFormStore()
  ...
  const onSubmit = async (values: FormValues) => {
    const confirmed = await swalConfirm({
      title: isEdit ? '¿Guardar cambios?' : '¿Crear tipo de sección?',
      text: values.typesection_name, confirmText: isEdit ? 'Sí, guardar' : 'Sí, crear', cancelText: 'Cancelar',
    })
    if (!confirmed) return
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4 max-w-lg">
          ...
          <CardContent className="pt-6 flex flex-col gap-4">
            <FormField control={form.control} name="typesection_name" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="Ej: Sala" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
        ...
        <div className="flex items-center justify-end gap-3">
          <Button type="button" variant="outline" onClick={() => router.push('/typesections')} disabled={isSubmitting}>Cancelar</Button>
