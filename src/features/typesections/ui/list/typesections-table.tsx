...
import { typesectionsColumns } from './typesections-columns'
import { TypeSectionStatsBar } from './typesection-stats-bar'

export function TypeSectionsTable() {
  const { items, meta, filters, hasLoaded, isInitialLoading, isFetching, isError, message, load, reset } =
    useTypeSectionListStore()
  const { bulkToggleState, bulkDeleteItems } = useTypeSectionDeleteStore()
  ...
  return (
    <div className="relative flex flex-1 flex-col gap-4">
      <TypeSectionStatsBar total={meta?.total ?? 0} active={activeCount} inactive={inactiveCount} />
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={typesectionsColumns.length} className="h-20 text-center text-sm text-muted-foreground">
