...
interface TypeSectionSelectProps {
  value?: number | null
...
export function TypeSectionSelect({
  value,
  onValueChange,
  placeholder = 'Seleccionar sección...',
  disabled,
  showAll = false,
}: TypeSectionSelectProps) {
  const [open, setOpen] = useState(false)
  const { options, isLoading, isError, load, setForceReload } = useTypeSectionSelectStore()

  useEffect(() => { load() }, [])

  const selected = value != null ? options.find((o) => o.id_typesection === value) : null
  const label    = selected ? selected.typesection_name : value === null && showAll ? 'Todos' : placeholder
        ...
        <Command>
          ...
          <CommandList>
            ...
            <CommandGroup>
              ...
              {options.map((opt) => (
                <CommandItem
                  key={opt.id_typesection}
                  value={opt.typesection_name}
                  onSelect={() => { onValueChange(opt.id_typesection); setOpen(false) }}
                >
                  <Check className={cn('mr-2 h-4 w-4', value === opt.id_typesection ? 'opacity-100' : 'opacity-0')} />
                  {opt.typesection_name}
                </CommandItem>
              ))}
            </CommandGroup>
          </CommandList>
