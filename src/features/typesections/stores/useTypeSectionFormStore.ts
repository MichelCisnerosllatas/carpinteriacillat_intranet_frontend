...
export const useTypeSectionFormStore = create<State & Action>((set) => ({
  ...
  update: async (id, data) => {
    ...
    try {
      ...
      const res = hasEmpty
        ? await typesectionsService.patch(id, Object.fromEntries(Object.entries(data).filter(([, v]) => v !== null && v !== undefined && v !== '')) as Partial<TypeSectionPutRequestDto>)
        : await typesectionsService.put(id, data)
      if (!res.success) { set({ isSubmitting: false, error: res.message, fieldErrors: res.errors ?? null }); return false }
      set({ isSubmitting: false })
      return true
    } catch (error: any) {
