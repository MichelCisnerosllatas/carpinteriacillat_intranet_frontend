...
import { sectionImagesColumns } from './sectionimages-columns'

export function SectionImagesTable() {
  const {
    items, meta, filters, hasLoaded, isInitialLoading, isFetching, isError, message,
    load, reset,
  } = useSectionImageListStore()
  const { bulkToggleState, bulkDeleteItems } = useSectionImageDeleteStore()
  ...
  const applyFilters = (overrides: { state?: string; dateFrom?: string; dateTo?: string }) => {
    ...
    setIsUserFetching(true)
    void load({ search, state: nextState === 'all' ? undefined : Number(nextState), date_from: nextDateFrom, date_to: nextDateTo, page: 1 }).finally(() => setIsUserFetching(false))
  }
  ...
  if (!hasLoaded && !isInitialLoading) {
    return (
      <div className="flex min-h-[300px] flex-col items-center justify-center">
        ...
        <p className="text-sm text-muted-foreground">Cargando sección — imágenes...</p>
      </div>
    )
  }
  ...
  if (isError) {
    return (
      <div className="flex min-h-[300px] flex-col items-center justify-center gap-3">
        <p className="text-sm font-semibold">Error al cargar registros</p>
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={sectionImagesColumns.length} className="h-20 text-center text-sm text-muted-foreground">
