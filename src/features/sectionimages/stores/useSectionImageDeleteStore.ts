...
export const useSectionImageDeleteStore = create<State & Action>((set) => ({
  isLoading: false,
  error: null,
  ...
  toggleState: async (id, newState) => {
    ...
    try {
      await sectionImagesService.patch(id, { sectionimage_state: newState })
  ...
  deleteItem: async (id) => {
    set({ isLoading: true, error: null })
    try {
      await sectionImagesService.delete(id)
      await useSectionImageListStore.getState().load()
      return true
    } catch {
  ...
  bulkToggleState: async (ids, newState) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => sectionImagesService.patch(id, { sectionimage_state: newState })))
      await useSectionImageListStore.getState().load()
      return true
    } catch {
  ...
  bulkDeleteItems: async (ids) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => sectionImagesService.delete(id)))
      await useSectionImageListStore.getState().load()
      return true
    } catch {
