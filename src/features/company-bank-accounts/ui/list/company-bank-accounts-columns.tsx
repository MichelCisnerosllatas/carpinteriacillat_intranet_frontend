...
import { CompanyBankAccountsRowActions } from './company-bank-accounts-row-actions'
import { BankLogo } from './bank-logo'

export const companyBankAccountsColumns: ColumnDef<CompanyBankAccount>[] = [
  {
    id: 'select',
    header: ({ table }) => (
      <Checkbox
  ...
  {
    id: 'info',
    accessorFn: (row) => `${row.bank} ${row.accountNumber} ${row.accountType ?? ''}`,
    header: ({ column }) => <DataTableColumnHeader column={column} title="Cuenta" />,
    cell: ({ row }) => {
      const { bank, accountNumber, accountType, logo } = row.original
      return (
...

  {
    ...
    cell: ({ row }) => (
      <span className="font-mono text-xs text-muted-foreground">{row.original.currency}</span>
    ),
    enableSorting: true,
    enableHiding: true,
...

  {
    ...
    cell: ({ row }) => {
      const opt = getStateOption(row.original.status)
      return (
