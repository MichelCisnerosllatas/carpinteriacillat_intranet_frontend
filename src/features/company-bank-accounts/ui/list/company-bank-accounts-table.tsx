...
import { COMPANY_BANK_ACCOUNT_CURRENCIES } from '../../data/data'
...
export function CompanyBankAccountsTable() {
  const {
    ...
    load, reset,
  } = useCompanyBankAccountListStore()
  const { bulkToggleState, bulkDeleteItems } = useCompanyBankAccountDeleteStore()
  ...
  const table = useReactTable({
    data: items,
    columns: companyBankAccountsColumns,
      ...
      <div className="flex items-end justify-between gap-2">
        <div className="flex flex-1 flex-wrap items-end gap-2">
          <div className="flex flex-col gap-1">
            ...
            <Input placeholder="Banco, cuenta, tipo o moneda..." value={search} disabled={isFetching} onChange={(e) => setSearch(e.target.value)} className="h-8 w-full sm:w-[240px]" />
          </div>
          <div className="flex flex-col gap-1">
          ...
          <div className="flex flex-col gap-1">
            ...
            <Select value={currency} disabled={isFetching} onValueChange={setCurrency}>
              ...
              <SelectContent>
                ...
                {COMPANY_BANK_ACCOUNT_CURRENCIES.map((c) => (
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={companyBankAccountsColumns.length} className="h-20 text-center text-sm text-muted-foreground">
