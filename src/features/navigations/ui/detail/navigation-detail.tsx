...
import { Pencil, Navigation2, CalendarDays, Hash, Link2 } from 'lucide-react'
...
export function NavigationDetail({ id }: { id: string }) {
  const router = useRouter()
  const { currentItem, items, setCurrentItem } = useNavigationListStore()

  useEffect(() => {
    if (!currentItem || String(currentItem.id) !== id) {
      const found = items.find((i) => String(i.id) === id)
      if (found) setCurrentItem(found)
      else router.replace('/navigations')
    }
  ...
  return (
    <div className="flex max-w-lg flex-col gap-4">
        ...
        <CardContent className="pt-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex flex-1 flex-col gap-1">
              ...
              {item.url && <p className="text-sm text-muted-foreground">{item.url}</p>}
              <Badge variant="outline" className={cn('mt-1 w-fit text-xs', stateOpt.badge)}>{stateOpt.label}</Badge>
            </div>
            <Button variant="outline" size="sm" onClick={() => { NProgress.start(); router.push(`/navigations/edit/${item.id}`) }}>
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2 text-sm"><Hash className="size-4" />Detalles</CardTitle></CardHeader>
        <CardContent className="flex flex-col gap-2 text-sm">
          {item.url && (
            <>
              <div className="flex items-center justify-between gap-2">
                <span className="flex items-center gap-1.5 text-muted-foreground"><Link2 className="size-3.5" />URL</span>
                <span className="font-mono text-xs font-medium">{item.url}</span>
              </div>
              <Separator />
            </>
          )}
          <div className="flex justify-between">
            ...
            <span className="font-medium tabular-nums">{item.order ?? '—'}</span>
          </div>
        </CardContent>
      </Card>
