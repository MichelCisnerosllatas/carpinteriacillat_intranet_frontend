...
export function NavigationsRowActions({ row }: { row: Row<Navigation> }) {
  const router = useRouter()
  const { setCurrentItem } = useNavigationListStore()
  const { toggleState, deleteItem } = useNavigationDeleteStore()
  ...
  const handleView = () => {
    ...
    NProgress.start()
    router.push(`/navigations/${row.original.id}`)
  }

  const handleEdit = () => {
    setCurrentItem(row.original)
    NProgress.start()
    router.push(`/navigations/edit/${row.original.id}`)
  }
  ...
  const handleToggleState = async () => {
    ...
    const confirmed = await swalConfirm({
      title: `¿${actionLabel} esta navegación?`,
    ...
    if (ok) toastSuccess(`Navegación ${resultLabel}`, `"${row.original.name}" fue ${resultLabel}.`)
  ...
  const handleDelete = async () => {
    ...
    if (ok) toastSuccess('Navegación eliminada', `"${row.original.name}" fue eliminada.`)
