...
import { useSectionItemDetailListStore } from './useSectionItemDetailListStore'
...
export const useSectionItemDetailDeleteStore = create<State & Action>((set) => ({
  isLoading: false,
  error: null,
  ...
  toggleState: async (id, newState) => {
    ...
    try {
      await sectionItemDetailsService.patch(id, { sectionitemdetail_state: newState })
  ...
  deleteItem: async (id) => {
    set({ isLoading: true, error: null })
    try {
      await sectionItemDetailsService.delete(id)
      await useSectionItemDetailListStore.getState().load()
      return true
    } catch {
  ...
  bulkToggleState: async (ids, newState) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => sectionItemDetailsService.patch(id, { sectionitemdetail_state: newState })))
      await useSectionItemDetailListStore.getState().load()
      return true
    } catch {
  ...
  bulkDeleteItems: async (ids) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => sectionItemDetailsService.delete(id)))
      await useSectionItemDetailListStore.getState().load()
      return true
    } catch {
