...
import { companyContactsColumns } from './company-contacts-columns'
import { CompanyContactStatsBar } from './company-contact-stats-bar'
...
export function CompanyContactsTable() {
  const { items, meta, filters, hasLoaded, isInitialLoading, isFetching, isError, message, load, reset } =
    useCompanyContactListStore()
  const { bulkToggleState, bulkDeleteItems } = useCompanyContactDeleteStore()
  ...
  const table = useReactTable({
    data: items,
    columns: companyContactsColumns,
  ...
  if (isError) {
    return (
      <div className="flex min-h-[300px] flex-col items-center justify-center gap-3">
        <p className="text-sm font-semibold">Error al cargar contactos</p>
      ...
      <div className="flex items-end justify-between gap-2">
        <div className="flex flex-1 flex-wrap items-end gap-2">
          <div className="flex flex-col gap-1">
            ...
            <Input placeholder="Nombre, teléfono o correo..." value={search} disabled={isFetching} onChange={(e) => setSearch(e.target.value)} className="h-8 w-full sm:w-[220px]" />
          </div>
          <div className="flex flex-col gap-1">
            ...
            <Select value={type} disabled={isFetching} onValueChange={setType}>
              ...
              <SelectContent>
                ...
                {CONTACT_TYPE_OPTIONS.map((t) => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="flex flex-col gap-1">
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={companyContactsColumns.length} className="h-20 text-center text-sm text-muted-foreground">
