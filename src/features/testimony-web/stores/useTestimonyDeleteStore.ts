...
import { useTestimonyListStore } from './useTestimonyListStore'
...
export const useTestimonyDeleteStore = create<State & Action>((set) => ({
  isLoading: false,
  error: null,
  ...
  toggleState: async (id, newState) => {
    ...
    try {
      await testimonyService.patch(id, { testimony_state: newState })
  ...
  bulkToggleState: async (ids, newState) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => testimonyService.patch(id, { testimony_state: newState })))
      await useTestimonyListStore.getState().load()
      return true
    } catch {
  ...
  bulkDeleteItems: async (ids) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => testimonyService.delete(id)))
      await useTestimonyListStore.getState().load()
      return true
    } catch {
