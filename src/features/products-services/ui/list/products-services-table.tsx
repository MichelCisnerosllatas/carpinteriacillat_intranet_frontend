...
import { useProductServiceDeleteStore } from '../../stores/useProductServiceDeleteStore'
import { productsServicesColumns } from './products-services-columns'
import { ProductServiceStatsBar } from './product-service-stats-bar'
...
export function ProductsServicesTable() {
  const { items, meta, filters, hasLoaded, isInitialLoading, isFetching, isError, message, load, reset } =
    useProductServiceListStore()
  const { bulkToggleState, bulkDeleteItems } = useProductServiceDeleteStore()
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void load({
        search,
        type: type === 'all' ? undefined : (type as 'product' | 'service'),
  ...
  const table = useReactTable({
    data: items,
    columns: productsServicesColumns,
  ...
  const resetFilters = () => {
    ...
    void load({ search: '', type: undefined, status: undefined, page: 1 })
  }
  ...
  if (!hasLoaded && !isInitialLoading) {
    return (
      <div className="flex min-h-[300px] flex-col items-center justify-center">
        ...
        <p className="text-sm text-muted-foreground">Cargando productos y servicios...</p>
      </div>
    )
  }
  ...
  if (isError) {
    return (
      <div className="flex min-h-[300px] flex-col items-center justify-center gap-3">
        <p className="text-sm font-semibold">Error al cargar productos y servicios</p>
  ...
  return (
    <div className="relative flex flex-1 flex-col gap-4">
      <ProductServiceStatsBar total={meta?.total ?? 0} products={productsCount} services={servicesCount} active={activeCount} />
          ...
          <div className="flex flex-col gap-1">
            ...
            <Select value={type} disabled={isFetching} onValueChange={setType}>
              ...
              <SelectContent>
                ...
                {PRODUCT_SERVICE_TYPES.map((t) => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="flex flex-col gap-1">
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={productsServicesColumns.length} className="h-20 text-center text-sm text-muted-foreground">
