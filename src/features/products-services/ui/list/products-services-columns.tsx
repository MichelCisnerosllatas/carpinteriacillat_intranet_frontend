...
export const productsServicesColumns: ColumnDef<ProductService>[] = [
...

  {
    ...
    cell: ({ row }) => (
      <Badge variant="secondary" className="text-xs font-normal">
        {getProductServiceTypeLabel(row.original.type)}
      </Badge>
    ),
    enableSorting: true,
    enableHiding: true,
  ...
  {
    id: 'furniture',
    header: ({ column }) => <DataTableColumnHeader column={column} title="Mueble vinculado" />,
    cell: ({ row }) => (
      row.original.furnitureName
        ? <Badge variant="outline" className="text-xs font-normal">{row.original.furnitureName}</Badge>
...

  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-col gap-0.5">
        <span className="text-sm font-medium tabular-nums">S/ {row.original.defaultPrice.toFixed(2)}</span>
        {row.original.unit && <span className="text-xs text-muted-foreground">{row.original.unit}</span>}
      </div>
    ),
    enableSorting: true,
    enableHiding: true,
