...
import { furnituresColumns } from './furnitures-columns'
...
export function FurnituresTable() {
  ...
  const { bulkToggleState, bulkDeleteItems } = useFurnitureDeleteStore()
  ...
  const handleBulkActivate = async () => {
    ...
    try {
      ...
      if (ok) { toastSuccess('Activados', `${selectedCount} mueble(s) activado(s).`); table.resetRowSelection() }
  ...
  const handleBulkDeactivate = async () => {
    ...
    try {
      ...
      if (ok) { toastSuccess('Desactivados', `${selectedCount} mueble(s) desactivado(s).`); table.resetRowSelection() }
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      if (ok) { toastSuccess('Eliminados', `${selectedCount} mueble(s) eliminado(s).`); table.resetRowSelection() }
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={furnituresColumns.length} className="h-20 text-center text-sm text-muted-foreground">
