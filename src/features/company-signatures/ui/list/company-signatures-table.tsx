...
import { companySignaturesColumns } from './company-signatures-columns'
import { CompanySignatureStatsBar } from './company-signature-stats-bar'
...
export function CompanySignaturesTable() {
  const { items, meta, filters, hasLoaded, isInitialLoading, isFetching, isError, message, load, reset } =
    useCompanySignatureListStore()
  const { bulkToggleState, bulkDeleteItems } = useCompanySignatureDeleteStore()
  ...
  const table = useReactTable({
    data: items,
    columns: companySignaturesColumns,
  ...
  const inactiveCount = items.filter((i) => i.statusValue !== 1).length
  ...
  if (isError) {
    return (
      <div className="flex min-h-[300px] flex-col items-center justify-center gap-3">
        <p className="text-sm font-semibold">Error al cargar firmas</p>
  ...
  return (
    <div className="relative flex flex-1 flex-col gap-4">
      <CompanySignatureStatsBar total={meta?.total ?? 0} active={activeCount} inactive={inactiveCount} />
      ...
      <div className="flex items-end justify-between gap-2">
        <div className="flex flex-1 flex-wrap items-end gap-2">
          <div className="flex flex-col gap-1">
            ...
            <Input placeholder="Nombre, cargo o teléfono..." value={search} disabled={isFetching} onChange={(e) => setSearch(e.target.value)} className="h-8 w-full sm:w-[220px]" />
          </div>
          <div className="flex flex-col gap-1">
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={companySignaturesColumns.length} className="h-20 text-center text-sm text-muted-foreground">
                  No hay firmas para mostrar.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
