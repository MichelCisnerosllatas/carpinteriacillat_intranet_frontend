...
import type { CompanySignatureListRequestDto, CompanySignatureApiItem } from '../model/companysignatureget.dto'
...
type State = {
  ...
  filters: CompanySignatureListRequestDto
  currentItem: CompanySignature | null
...
type Action = {
  ...
  load: (params?: CompanySignatureListRequestDto) => Promise<boolean>
  setCurrentItem: (item: CompanySignature | null) => void
  reset: () => void
}

const defaultFilters: CompanySignatureListRequestDto = {
  page: 1, per_page: 10, search: '', status: undefined,
}

const mapFromApi = (item: CompanySignatureApiItem): CompanySignature => {
...
export const useCompanySignatureListStore = create<State & Action>((set, get) => ({
  ...
  load: async (params = {}) => {
    ...
    try {
      const response = await companySignaturesService.getList(nextFilters)
