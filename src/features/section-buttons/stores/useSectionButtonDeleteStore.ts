...
import { useSectionButtonListStore } from './useSectionButtonListStore'
...
export const useSectionButtonDeleteStore = create<State & Action>((set) => ({
  isLoading: false,
  error: null,
  ...
  toggleState: async (id, newState) => {
    ...
    try {
      await sectionButtonsService.patch(id, { sectionbutton_state: newState })
  ...
  deleteItem: async (id) => {
    set({ isLoading: true, error: null })
    try {
      await sectionButtonsService.delete(id)
      await useSectionButtonListStore.getState().load()
      return true
    } catch {
  ...
  bulkToggleState: async (ids, newState) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => sectionButtonsService.patch(id, { sectionbutton_state: newState })))
      await useSectionButtonListStore.getState().load()
      return true
    } catch {
  ...
  bulkDeleteItems: async (ids) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => sectionButtonsService.delete(id)))
      await useSectionButtonListStore.getState().load()
      return true
    } catch {
