...
import { getSectionButtonsColumns, getSectionButtonsScopedColumns } from './section-buttons-columns'
...
interface SectionButtonsTableProps {
  /** Opcional: sin él, la tabla carga todos los botones (uso standalone en `/section-buttons`);
   * con él, filtra por esa sección y oculta la columna "Sección" — se usa embebida en un tab
   * del detalle de `Section`. */
  idSection?: number
  /** `type_section.web_settings.buttons_delete` — oculta "Eliminar" (por fila y masivo).
   * Default `true` (comportamiento de siempre). Crear se controla aparte, ocultando el botón
   * "Nuevo botón" en `SectionDetailButtonsTab`. */
  canDelete?: boolean
  /** `type_section.web_settings.buttons_reorder` — oculta la columna "Orden". Default `true`
   * (comportamiento de siempre). */
  canReorder?: boolean
}

/**
 * `idSection` es opcional: sin él, la tabla carga todos los botones (uso standalone en
 * `/section-buttons`); con él, filtra por esa sección y oculta la columna "Sección" — se usa
 * embebida en un tab del detalle de `Section`.
 */
export function SectionButtonsTable({ idSection, canDelete = true, canReorder = true }: SectionButtonsTableProps) {
  const {
    items, meta, filters, hasLoaded, isInitialLoading, isFetching, isError, message,
    load, reset,
  } = useSectionButtonListStore()
  const { bulkToggleState, bulkDeleteItems } = useSectionButtonDeleteStore()

  const columns = useMemo(
    () => (idSection !== undefined
      ? getSectionButtonsScopedColumns({ hideDelete: !canDelete, hideOrder: !canReorder })
      : getSectionButtonsColumns({ hideDelete: !canDelete })),
    [idSection, canDelete, canReorder]
  )
  ...
  useEffect(() => { void load({ id_section: idSection }) }, [idSection])
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ...
      void load({ id_section: idSection, search, state: state === 'all' ? undefined : Number(state), page: 1 }).finally(() => setIsUserFetching(false))
    }, 500)
  ...
  const handleStateChange = (value: string) => {
    setState(value)
    setIsUserFetching(true)
    void load({ id_section: idSection, search, state: value === 'all' ? undefined : Number(value), page: 1 }).finally(() => setIsUserFetching(false))
  }
  ...
  const resetFilters = () => {
    ...
    setIsUserFetching(true)
    void load({ id_section: idSection, search: '', state: undefined, page: 1 }).finally(() => setIsUserFetching(false))
  }
      ...
      <div className="flex items-end justify-between gap-2">
        <div className="flex flex-1 flex-wrap items-end gap-2">
          <div className="flex flex-col gap-1">
            ...
            <Input placeholder="Etiqueta del botón..." value={search} disabled={isFetching} onChange={(e) => setSearch(e.target.value)} className="h-8 w-full sm:w-[220px]" />
          </div>
          <div className="flex flex-col gap-1">
