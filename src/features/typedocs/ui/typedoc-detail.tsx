...
import { Pencil, FileText, CalendarDays } from 'lucide-react'
...
export function TypeDocDetail({ id }: { id: string }) {
  const router = useRouter()
  const { currentItem, items, setCurrentItem } = useTypeDocListStore()

  useEffect(() => {
    if (!currentItem || String(currentItem.id) !== id) {
      const found = items.find((i) => String(i.id) === id)
      if (found) setCurrentItem(found)
      else router.replace('/typedocs')
    }
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-start gap-4">
            ...
            <Button variant="outline" size="sm" onClick={() => { NProgress.start(); router.push(`/typedocs/edit/${item.id}`) }}>
