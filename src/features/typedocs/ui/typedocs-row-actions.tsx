...
export function TypeDocsRowActions({ row }: { row: Row<TypeDoc> }) {
  const router = useRouter()
  const { setCurrentItem } = useTypeDocListStore()
  const { toggleState, deleteItem } = useTypeDocDeleteStore()
  ...
  const handleView = () => {
    ...
    NProgress.start()
    router.push(`/typedocs/${row.original.id}`)
  }

  const handleEdit = () => {
    setCurrentItem(row.original)
    NProgress.start()
    router.push(`/typedocs/edit/${row.original.id}`)
  }
