...
import { useTypeDocSelectStore } from '../stores/useTypeDocSelectStore'
...
export function TypeDocSelect({
  ...
  const { options, isLoading, isError, load, setForceReload } = useTypeDocSelectStore()

  useEffect(() => { load() }, [])

  const selected = value != null ? options.find((o) => o.id_typedoc === value) : null
  const label    = selected ? selected.typedoc_name : value === null && showAll ? 'Todos' : placeholder
          ...
          <CommandList>
            ...
            <CommandGroup>
              ...
              {options.map((opt) => (
                <CommandItem
                  key={opt.id_typedoc}
                  value={opt.typedoc_name}
                  onSelect={() => { onValueChange(opt.id_typedoc); setOpen(false) }}
                >
                  <Check className={cn('mr-2 h-4 w-4', value === opt.id_typedoc ? 'opacity-100' : 'opacity-0')} />
                  {opt.typedoc_name}
                </CommandItem>
              ))}
            </CommandGroup>
