...
import { useTypeDocListStore } from './useTypeDocListStore'
...
export const useTypeDocDeleteStore = create<State & Action>((set) => ({
  isLoading: false,
  error: null,
  ...
  toggleState: async (id, newState) => {
    ...
    try {
      await typedocsService.patch(id, { typedoc_state: newState })
  ...
  deleteItem: async (id) => {
    set({ isLoading: true, error: null })
    try {
      await typedocsService.delete(id)
      await useTypeDocListStore.getState().load()
      return true
    } catch {
  ...
  bulkToggleState: async (ids, newState) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => typedocsService.patch(id, { typedoc_state: newState })))
      await useTypeDocListStore.getState().load()
      return true
    } catch {
  ...
  bulkDeleteItems: async (ids) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => typedocsService.delete(id)))
      await useTypeDocListStore.getState().load()
      return true
    } catch {
