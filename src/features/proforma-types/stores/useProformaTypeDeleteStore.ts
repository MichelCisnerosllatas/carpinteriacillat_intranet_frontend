...
import { useProformaTypeListStore } from './useProformaTypeListStore'
...
export const useProformaTypeDeleteStore = create<State & Action>((set) => ({
  isLoading: false,
  error: null,
  ...
  toggleState: async (id, newState) => {
    ...
    try {
      await proformaTypesService.patch(id, { status: newState })
  ...
  deleteItem: async (id) => {
    set({ isLoading: true, error: null })
    try {
      await proformaTypesService.delete(id)
      await useProformaTypeListStore.getState().load()
      return true
    } catch {
  ...
  bulkToggleState: async (ids, newState) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => proformaTypesService.patch(id, { status: newState })))
      await useProformaTypeListStore.getState().load()
      return true
    } catch {
  ...
  bulkDeleteItems: async (ids) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => proformaTypesService.delete(id)))
      await useProformaTypeListStore.getState().load()
      return true
    } catch {
