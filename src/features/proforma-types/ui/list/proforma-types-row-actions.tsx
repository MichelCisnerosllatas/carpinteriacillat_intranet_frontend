...
export function ProformaTypesRowActions({ row }: { row: Row<ProformaType> }) {
  const router = useRouter()
  const { setCurrentItem } = useProformaTypeListStore()
  const { toggleState, deleteItem } = useProformaTypeDeleteStore()
  ...
  const handleView = () => {
    ...
    NProgress.start()
    router.push(`/proforma-types/${row.original.id}`)
  }

  const handleEdit = () => {
    setCurrentItem(row.original)
    NProgress.start()
    router.push(`/proforma-types/edit/${row.original.id}`)
  }
