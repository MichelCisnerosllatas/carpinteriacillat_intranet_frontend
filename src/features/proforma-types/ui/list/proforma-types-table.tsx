...
import { useProformaTypeDeleteStore } from '../../stores/useProformaTypeDeleteStore'
import { proformaTypesColumns } from './proforma-types-columns'
import { ProformaTypeStatsBar } from './proforma-type-stats-bar'

export function ProformaTypesTable() {
  const { items, meta, filters, hasLoaded, isInitialLoading, isFetching, isError, message, load, reset } =
    useProformaTypeListStore()
  const { bulkToggleState, bulkDeleteItems } = useProformaTypeDeleteStore()
  ...
  if (!hasLoaded && !isInitialLoading) {
    return (
      <div className="flex min-h-[300px] flex-col items-center justify-center">
        ...
        <p className="text-sm text-muted-foreground">Cargando tipos de proforma...</p>
      </div>
    )
  }
  ...
  if (isError) {
    return (
      <div className="flex min-h-[300px] flex-col items-center justify-center gap-3">
        <p className="text-sm font-semibold">Error al cargar tipos de proforma</p>
  ...
  return (
    <div className="relative flex flex-1 flex-col gap-4">
      <ProformaTypeStatsBar total={meta?.total ?? 0} active={activeCount} inactive={inactiveCount} />
      ...
      <div className="flex items-end justify-between gap-2">
        <div className="flex flex-1 flex-wrap items-end gap-2">
          <div className="flex flex-col gap-1">
            ...
            <Input placeholder="Nombre o código..." value={search} disabled={isFetching} onChange={(e) => setSearch(e.target.value)} className="h-8 w-full sm:w-[220px]" />
          </div>
          <div className="flex flex-col gap-1">
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={proformaTypesColumns.length} className="h-20 text-center text-sm text-muted-foreground">
                  No hay tipos de proforma para mostrar.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
