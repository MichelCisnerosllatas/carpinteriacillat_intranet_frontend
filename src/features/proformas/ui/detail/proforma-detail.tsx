  ...
  const handleChangeStatus = async (newStatus: ProformaStatus) => {
    ...
    await swalConfirmAction({
      ...
      action: async ({ close, showError }) => {
        ...
        if (ok) {
          toastSuccess('Estado actualizado', `"${item.code}" ahora está ${opt.label.toLowerCase()}.`)
    ...
    <div className="flex flex-col gap-4">
        ...
        <CardContent className="flex flex-col gap-4 pt-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-sm text-muted-foreground">Emitida el {item.issueDate}</p>
              {item.dueDate && <p className="text-sm text-muted-foreground">Vence el {item.dueDate}</p>}
            </div>
            <div className="flex flex-col items-end gap-2">
              <Badge variant="outline" className={cn('text-sm', statusOpt.badge)}>{statusOpt.label}</Badge>
              <Button variant="outline" size="sm" onClick={() => { NProgress.start(); router.push(`/proformas/edit/${item.id}`) }}>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-1">
              <span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Cliente</span>
              ...
              {item.clientAddress && <span className="text-xs text-muted-foreground">{item.clientAddress}</span>}
              {item.clientAttention && <span className="text-xs text-muted-foreground">Atención: {item.clientAttention}</span>}
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Datos de la proforma</span>
              <span className="text-xs text-muted-foreground">Lugar de emisión: {item.placeOfIssue ?? '—'}</span>
              <span className="text-xs text-muted-foreground">Plazo de entrega: {item.deliveryTime ?? '—'}</span>
              <span className="text-xs text-muted-foreground">Moneda: {item.currency}</span>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                {item.details.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} className="h-20 text-center text-sm text-muted-foreground">Sin líneas.</TableCell>
                  </TableRow>
                ) : (
                  item.details.map((d) => (
                    <TableRow key={d.id}>
                      ...
                      <TableCell className="text-right font-medium">{formatCurrency(d.total, item.currency)}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
          ...
          <div className="ml-auto mt-4 flex w-full max-w-xs flex-col gap-1 rounded-lg border p-4 text-sm">
            <div className="flex justify-between"><span className="text-muted-foreground">Subtotal</span><span className="font-medium">{formatCurrency(item.subtotal, item.currency)}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">Impuestos</span><span className="font-medium">{formatCurrency(item.tax, item.currency)}</span></div>
            <div className="mt-1 flex justify-between border-t pt-1 text-lg"><span className="font-semibold">Total</span><span className="font-bold">{formatCurrency(item.total, item.currency)}</span></div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" disabled={isDownloading} onClick={() => void handleDownload(true)}>
            {isDownloading ? <Loader2 className="size-4 mr-1 animate-spin" /> : <Download className="size-4 mr-1" />}
            Descargar PDF
          </Button>
          <Button variant="outline" size="sm" disabled={isViewing} onClick={() => void handleDownload(false)}>
            {isViewing ? <Loader2 className="size-4 mr-1 animate-spin" /> : <Eye className="size-4 mr-1" />}
            Ver PDF
          </Button>
          ...
          {transitions.length > 0 && (
            <>
              ...
              {transitions.map((t) => (
                <Button key={t.value} variant="outline" size="sm" onClick={() => void handleChangeStatus(t.value)}>
                  {t.label}
                </Button>
              ))}
            </>
