...
export function CompanyBranchesTable() {
  const { items, meta, filters, hasLoaded, isInitialLoading, isFetching, isError, message, load, reset } =
    useCompanyBranchListStore()
  const { bulkToggleState, bulkDeleteItems } = useCompanyBranchDeleteStore()
  ...
  const table = useReactTable({
    data: items,
    columns: companyBranchesColumns,
  ...
  return (
    <div className="relative flex flex-1 flex-col gap-4">
      <CompanyBranchStatsBar total={meta?.total ?? 0} active={activeCount} inactive={inactiveCount} />
      ...
      <div className="flex items-end justify-between gap-2">
        <div className="flex flex-1 flex-wrap items-end gap-2">
          <div className="flex flex-col gap-1">
            ...
            <Input placeholder="Nombre o dirección..." value={search} disabled={isFetching} onChange={(e) => setSearch(e.target.value)} className="h-8 w-full sm:w-[220px]" />
          </div>
          <div className="flex flex-col gap-1">
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={companyBranchesColumns.length} className="h-20 text-center text-sm text-muted-foreground">
