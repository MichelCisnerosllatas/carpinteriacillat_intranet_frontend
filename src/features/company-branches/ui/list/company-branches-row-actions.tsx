...
export function CompanyBranchesRowActions({ row }: { row: Row<CompanyBranch> }) {
  const router = useRouter()
  const { setCurrentItem } = useCompanyBranchListStore()
  const { toggleState, deleteItem } = useCompanyBranchDeleteStore()
  ...
  const handleView = () => {
    ...
    NProgress.start()
    router.push(`/company-branches/${row.original.id}`)
  }

  const handleEdit = () => {
    setCurrentItem(row.original)
    NProgress.start()
    router.push(`/company-branches/edit/${row.original.id}`)
  }
