...
import { useSaleDocumentTypeSelectStore } from '../stores/useSaleDocumentTypeSelectStore'
...
interface SaleDocumentTypeSelectProps {
  value?: number | null
...
export function SaleDocumentTypeSelect({
  value,
  onValueChange,
  placeholder = 'Seleccionar tipo de comprobante...',
  disabled,
  showAll = false,
}: SaleDocumentTypeSelectProps) {
  const [open, setOpen] = useState(false)
  const { options, isLoading, isError, load, setForceReload } = useSaleDocumentTypeSelectStore()
  ...
  const [fallback, setFallback] = useState<SaleDocumentTypeApiItem | null>(null)
  ...
  useEffect(() => {
    ...
    let cancelled = false
    saleDocumentTypesService.getById(value).then((res) => {
