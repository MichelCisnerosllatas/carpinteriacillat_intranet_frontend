...
export function SaleDocumentTypesRowActions({ row }: { row: Row<SaleDocumentType> }) {
  const router = useRouter()
  const { setCurrentItem } = useSaleDocumentTypeListStore()
  const { toggleState, deleteItem } = useSaleDocumentTypeDeleteStore()
  ...
  const handleView = () => {
    ...
    NProgress.start()
    router.push(`/sale-document-types/${row.original.id}`)
  }

  const handleEdit = () => {
    setCurrentItem(row.original)
    NProgress.start()
    router.push(`/sale-document-types/edit/${row.original.id}`)
  }
