...
import { useSaleDocumentTypeDeleteStore } from '../../stores/useSaleDocumentTypeDeleteStore'
import { saleDocumentTypesColumns } from './sale-document-types-columns'
import { SaleDocumentTypeStatsBar } from './sale-document-type-stats-bar'

export function SaleDocumentTypesTable() {
  const { items, meta, filters, hasLoaded, isInitialLoading, isFetching, isError, message, load, reset } =
    useSaleDocumentTypeListStore()
  const { bulkToggleState, bulkDeleteItems } = useSaleDocumentTypeDeleteStore()
  ...
  const table = useReactTable({
    data: items,
    columns: saleDocumentTypesColumns,
  ...
  if (!hasLoaded && !isInitialLoading) {
    return (
      <div className="flex min-h-[300px] flex-col items-center justify-center">
        ...
        <p className="text-sm text-muted-foreground">Cargando tipos de comprobante...</p>
      </div>
    )
  }
  ...
  if (isError) {
    return (
      <div className="flex min-h-[300px] flex-col items-center justify-center gap-3">
        <p className="text-sm font-semibold">Error al cargar tipos de comprobante</p>
  ...
  return (
    <div className="relative flex flex-1 flex-col gap-4">
      <SaleDocumentTypeStatsBar total={meta?.total ?? 0} active={activeCount} inactive={inactiveCount} />
      ...
      <div className="flex items-end justify-between gap-2">
        <div className="flex flex-1 flex-wrap items-end gap-2">
          <div className="flex flex-col gap-1">
            ...
            <Input placeholder="Nombre, código o serie..." value={search} disabled={isFetching} onChange={(e) => setSearch(e.target.value)} className="h-8 w-full sm:w-[220px]" />
          </div>
          <div className="flex flex-col gap-1">
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={saleDocumentTypesColumns.length} className="h-20 text-center text-sm text-muted-foreground">
                  No hay tipos de comprobante para mostrar.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
