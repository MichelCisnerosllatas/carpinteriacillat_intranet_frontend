...
import { useSectionListStore } from './useSectionListStore'
...
export const useSectionDeleteStore = create<State & Action>((set) => ({
  isLoading: false,
  error: null,
  ...
  toggleState: async (id, newState) => {
    ...
    try {
      await sectionsService.patch(id, { section_state: newState })
  ...
  bulkToggleState: async (ids, newState) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => sectionsService.patch(id, { section_state: newState })))
      await useSectionListStore.getState().load()
      return true
    } catch {
  ...
  bulkDeleteItems: async (ids) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => sectionsService.delete(id)))
      await useSectionListStore.getState().load()
      return true
    } catch {
