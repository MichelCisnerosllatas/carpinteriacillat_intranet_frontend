...
import { useTypeColorSelectStore } from '../stores/useTypeColorSelectStore'

interface TypeColorSelectProps {
  value?: number | null
...
export function TypeColorSelect({
  value,
  onValueChange,
  ...
  const selected = value != null ? options.find((o) => o.id_typecolor === value) : null
  const label    = selected ? selected.typecolor_name : value === null && showAll ? 'Todos' : placeholder
        ...
        <Command>
          ...
          <CommandList>
            ...
            <CommandGroup>
              ...
              {options.map((opt) => (
                <CommandItem
                  key={opt.id_typecolor}
                  value={opt.typecolor_name}
                  onSelect={() => { onValueChange(opt.id_typecolor); setOpen(false) }}
                >
                  <Check className={cn('mr-2 h-4 w-4', value === opt.id_typecolor ? 'opacity-100' : 'opacity-0')} />
                  {opt.typecolor_name}
                </CommandItem>
              ))}
            </CommandGroup>
