// src/features/typecolors/ui/list/typecolors-row-actions.tsx
'use client'
...
export function TypeColorsRowActions({ row }: { row: Row<TypeColor> }) {
  const router = useRouter()
  const { setCurrentItem } = useTypeColorListStore()
  const { toggleState, deleteItem } = useTypeColorDeleteStore()
  ...
  const handleView = () => {
    ...
    NProgress.start()
    router.push(`/typecolors/${row.original.id}`)
  }

  const handleEdit = () => {
    setCurrentItem(row.original)
    NProgress.start()
    router.push(`/typecolors/edit/${row.original.id}`)
  }
  ...
  const handleToggleState = async () => {
    ...
    if (ok) toastSuccess(`Color ${resultLabel}`, `"${row.original.name}" fue ${resultLabel}.`)
  ...
  const handleDelete = async () => {
    ...
    if (ok) toastSuccess('Color eliminado', `"${row.original.name}" fue eliminado.`)
