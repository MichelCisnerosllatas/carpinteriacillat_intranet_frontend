...
import type { ProformaTemplatePostRequestDto, ProformaTemplatePostResponseDto } from '../model/proformatemplatepost.dto'
import type { ProformaTemplatePutRequestDto, ProformaTemplatePutResponseDto } from '../model/proformatemplateput.dto'
...
export const proformaTemplatesService = {
  ...
  getById: async (id: number): Promise<ProformaTemplateGetByIdResponseDto> => {
    const { data } = await apiClient.get<ProformaTemplateGetByIdResponseDto>(`${PROFORMA_TEMPLATES_ENDPOINTS.v1.getJoin}/${id}`)
    return data
  },
  ...
  post: async (param: ProformaTemplatePostRequestDto): Promise<ProformaTemplatePostResponseDto> => {
    const { data } = await apiClient.post<ProformaTemplatePostResponseDto>(PROFORMA_TEMPLATES_ENDPOINTS.v1.post, param)
    return data
  },

  put: async (id: number, param: ProformaTemplatePutRequestDto): Promise<ProformaTemplatePutResponseDto> => {
    const { data } = await apiClient.put<ProformaTemplatePutResponseDto>(PROFORMA_TEMPLATES_ENDPOINTS.v1.put(id), param)
    return data
  },

  patch: async (id: number, param: Partial<ProformaTemplatePutRequestDto>): Promise<ProformaTemplatePutResponseDto> => {
    const { data } = await apiClient.patch<ProformaTemplatePutResponseDto>(PROFORMA_TEMPLATES_ENDPOINTS.v1.patch(id), param)
    return data
  },
