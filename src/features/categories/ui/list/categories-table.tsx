...
export function CategoriesTable() {
  ...
  const { bulkToggleState, bulkDeleteItems } = useCategoryDeleteStore()
  ...
  const handleBulkActivate = async () => {
    ...
    try {
      ...
      if (ok) { toastSuccess('Activadas', `${selectedCount} categoría(s) activada(s).`); table.resetRowSelection() }
      else toastError('Error', 'No se pudieron activar todas las categorías.')
  ...
  const handleBulkDeactivate = async () => {
    ...
    try {
      ...
      if (ok) { toastSuccess('Desactivadas', `${selectedCount} categoría(s) desactivada(s).`); table.resetRowSelection() }
      else toastError('Error', 'No se pudieron desactivar todas las categorías.')
    } finally { setIsBulkLoading(false) }
  }
  ...
  return (
    <div className="relative flex flex-1 flex-col gap-4">
      <CategoryStatsBar total={meta?.total ?? 0} active={activeCount} inactive={inactiveCount} />
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={categoriesColumns.length} className="h-20 text-center text-sm text-muted-foreground">
