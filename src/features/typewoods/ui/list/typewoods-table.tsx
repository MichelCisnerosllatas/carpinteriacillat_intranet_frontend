...
export function TypeWoodsTable() {
  const { items, meta, filters, hasLoaded, isInitialLoading, isFetching, isError, message, load, reset } =
    useTypeWoodListStore()
  const { bulkToggleState, bulkDeleteItems } = useTypeWoodDeleteStore()
  ...
  const handleBulkDelete = async () => {
    ...
    if (ok) { toastSuccess('Eliminadas', `${selectedCount} registro(s) eliminado(s).`); table.resetRowSelection() }
  ...
  return (
    <div className="relative flex flex-1 flex-col gap-4">
      <TypeWoodStatsBar total={meta?.total ?? 0} active={activeCount} inactive={inactiveCount} />
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={typewoodsColumns.length} className="h-20 text-center text-sm text-muted-foreground">
                  No hay maderas para mostrar.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
