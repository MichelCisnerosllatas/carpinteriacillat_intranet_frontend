...
import { TypeWoodsRowActions } from './typewoods-row-actions'

export const typewoodsColumns: ColumnDef<TypeWood>[] = [
  {
    id: 'select',
    header: ({ table }) => (
      <Checkbox
...

  {
    ...
    header: ({ column }) => <DataTableColumnHeader column={column} title="Madera" />,
    cell: ({ row }) => (
      <div className="flex min-w-[200px] flex-col gap-0.5 py-1.5">
        <Link href={`/typewoods/${row.original.id}`} className="text-sm font-medium leading-none text-primary hover:underline">{row.original.name}</Link>
