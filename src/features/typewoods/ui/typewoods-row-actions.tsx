...
export function TypeWoodsRowActions({ row }: { row: Row<TypeWood> }) {
  const router = useRouter()
  const { setCurrentItem } = useTypeWoodListStore()
  const { toggleState, deleteItem } = useTypeWoodDeleteStore()
  ...
  const handleView = () => {
    ...
    NProgress.start()
    router.push(`/typewoods/${row.original.id}`)
  }

  const handleEdit = () => {
    setCurrentItem(row.original)
    NProgress.start()
    router.push(`/typewoods/edit/${row.original.id}`)
  }
  ...
  const handleToggleState = async () => {
    ...
    if (ok) toastSuccess(`Registro ${resultLabel}`, `"${row.original.name}" fue ${resultLabel}.`)
  ...
  const handleDelete = async () => {
    ...
    if (ok) toastSuccess('Registro eliminado', `"${row.original.name}" fue eliminado.`)
