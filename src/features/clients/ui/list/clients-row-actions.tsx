...
export function ClientsRowActions({ row }: { row: Row<Client> }) {
  const router = useRouter()
  const { setCurrentItem } = useClientListStore()
  const { toggleState, deleteItem } = useClientDeleteStore()
  ...
  const handleEdit = () => {
    setCurrentItem(row.original)
    NProgress.start()
    router.push(`/clients/edit/${row.original.id}`)
  }
  ...
  const handleToggleState = async () => {
    ...
    await swalConfirmAction({
      title: `¿${actionLabel.charAt(0).toUpperCase() + actionLabel.slice(1)} este cliente?`,
      text: row.original.businessName,
      ...
      action: async ({ close, showError }) => {
        ...
        if (ok) {
          toastSuccess(`Cliente ${resultLabel}`, `"${row.original.businessName}" fue ${resultLabel}.`)
          close()
        } else {
  ...
  const handleDelete = async () => {
    await swalDeleteConfirm(
      `¿Eliminar "${row.original.businessName}"?`, 'Esta acción no se puede deshacer.',
      async ({ close, showError }) => {
        const ok = await deleteItem(row.original.id)
        if (ok) {
          toastSuccess('Cliente eliminado', `"${row.original.businessName}" fue eliminado.`)
          close()
        } else {
