...
import { Pencil, Building2, CalendarDays, Mail, MapPin, Phone, User } from 'lucide-react'
...
export function ClientDetail({ id }: { id: string }) {
  ...
  const { currentItem, items, setCurrentItem, loadById } = useClientListStore()
    ...
    <div className="flex max-w-lg flex-col gap-4">
            ...
            <div className="flex flex-1 flex-col gap-1">
              ...
              {item.typedocName && (
                <p className="text-sm text-muted-foreground">
                  {item.typedocName}{item.documentNumber ? ` · ${item.documentNumber}` : ''}
                </p>
              )}
              <Badge variant="outline" className={cn('mt-1 w-fit text-xs', stateOpt.badge)}>{stateOpt.label}</Badge>
            </div>
            <Button variant="outline" size="sm" onClick={() => { NProgress.start(); router.push(`/clients/edit/${item.id}`) }}>
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-2 text-sm">
          <div className="flex items-center gap-2"><User className="size-4 text-muted-foreground" /><span>{item.contactPerson || '—'}</span></div>
          <div className="flex items-center gap-2"><Phone className="size-4 text-muted-foreground" /><span>{item.phone || '—'}</span></div>
          <div className="flex items-center gap-2"><Mail className="size-4 text-muted-foreground" /><span>{item.email || '—'}</span></div>
          <div className="flex items-center gap-2"><MapPin className="size-4 text-muted-foreground" /><span>{item.address || '—'}</span></div>
        </CardContent>
      </Card>
