...
export function TypeFontsRowActions({ row }: { row: Row<TypeFont> }) {
  const router = useRouter()
  const { setCurrentItem } = useTypeFontListStore()
  const { toggleState, deleteItem } = useTypeFontDeleteStore()
  ...
  const handleView = () => {
    ...
    NProgress.start()
    router.push(`/typefonts/${row.original.id}`)
  }

  const handleEdit = () => {
    setCurrentItem(row.original)
    NProgress.start()
    router.push(`/typefonts/edit/${row.original.id}`)
  }
