...
import { Pencil, Type, CalendarDays } from 'lucide-react'
...
export function TypeFontDetail({ id }: { id: string }) {
  ...
  return (
    <div className="flex max-w-lg flex-col gap-4">
        ...
        <CardContent className="pt-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex flex-1 flex-col gap-1">
              <h3 className="text-lg font-semibold" style={fontFamilyStyle(item.name)}>{item.name}</h3>
              {item.description && <p className="text-sm text-muted-foreground">{item.description}</p>}
              <Badge variant="outline" className={cn('mt-1 w-fit text-xs', stateOpt.badge)}>{stateOpt.label}</Badge>
            </div>
            <Button variant="outline" size="sm" onClick={() => { NProgress.start(); router.push(`/typefonts/edit/${item.id}`) }}>
      ...
      <TypeFontPreview family={item.name} />
