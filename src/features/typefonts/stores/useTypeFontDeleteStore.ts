...
import { useTypeFontListStore } from './useTypeFontListStore'
...
export const useTypeFontDeleteStore = create<State & Action>((set) => ({
  isLoading: false,
  error: null,
  ...
  toggleState: async (id, newState) => {
    ...
    try {
      await typefontsService.patch(id, { typefont_state: newState })
  ...
  deleteItem: async (id) => {
    set({ isLoading: true, error: null })
    try {
      await typefontsService.delete(id)
      await useTypeFontListStore.getState().load()
      return true
    } catch {
  ...
  bulkToggleState: async (ids, newState) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => typefontsService.patch(id, { typefont_state: newState })))
      await useTypeFontListStore.getState().load()
      return true
    } catch {
  ...
  bulkDeleteItems: async (ids) => {
    set({ isLoading: true, error: null })
    try {
      await Promise.all(ids.map((id) => typefontsService.delete(id)))
      await useTypeFontListStore.getState().load()
      return true
    } catch {
